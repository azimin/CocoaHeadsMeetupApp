 ...
 * Easily check
 ...
 `if permissions.isAllowed(type: .camera) { ... }`
 
 * Request needed permission
 
 `permissions.requestAccess(forType: .camera) { granted in }`
 ...
 * You can also get default-styled `UIAlertController` for presenting while requesting rejected permission
 ...
 `let alert = permissions.alertForSettingsWith(type: .camera)`
...
class PermissionsManager {
  ...
  var availableTypes: [PermissionType] {
    return [.notifications, .calendar, .reminder, .camera, .photosLibrary]
  }
  ...
  */
  func isAllowed(type: PermissionType) -> Bool {
    switch type {
      case .photosLibrary:
  ...
  */
  func requestAccess(forType: PermissionType, completion: @escaping (Bool) -> Void) {
  ...
  */
  func alertForSettingsWith(type: PermissionType) -> UIAlertController {
    var phrase = ""
    switch type {
      case .calendar: phrase = "к календарю"
      case .camera: phrase = "к камере"
      case .notifications: phrase = "к отправке уведомлений"
      case .photosLibrary: phrase = "к библиотеке фотографий"
      case .reminder: phrase = "к напоминаниям"
    }
    let message = "Пожалуйста, предоставьте приложению доступ \(phrase)"
    let alert = UIAlertController(title: "Ошибка доступа", message: message, preferredStyle: .alert)
  ...
  */
  ...
  /// Self-protection
  private func checkDescriptionKey(forType: PermissionType) {
  ...
  private func describedAssert(plist: UnsafeMutablePointer<[String: AnyObject]>, key: String) {
    assert(plist.pointee.keys.contains(key), "Key \(key) if required to be described in Info.plist")
  }
}
